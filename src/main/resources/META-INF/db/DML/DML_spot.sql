...
SELECT * 
    FROM (SELECT ROWNUM RN, A.* FROM (SELECT S.*, NVL(bCnt, 0) bCnt, L.LNAME FROM SPOT S, LOCATION L, (SELECT SNAME, COUNT(*) bCnt FROM BOOKMARK GROUP BY SNAME)B 
        WHERE S.SNAME = B.SNAME(+) AND S.LOCATIONNO = L.LOCATIONNO)A)
WHERE RN BETWEEN 1 AND 3;

SELECT S.*, L.LNAME FROM SPOT S, LOCATION L WHERE S.LOCATIONNO = L.LOCATIONNO;
...
SELECT c.*, (SELECT COUNT(*) FROM BOOKMARK WHERE MID ='kim' AND SNAME = C.sname) bookmarkOk
    FROM (SELECT ROWNUM RN, A.* FROM (SELECT S.*, NVL(bCnt, 0) bCnt, L.LNAME FROM SPOT S, LOCATION L, (SELECT SNAME, COUNT(*) bCnt FROM BOOKMARK GROUP BY SNAME)B 
        WHERE S.SNAME = B.SNAME(+) AND S.LOCATIONNO = L.LOCATIONNO AND S.SNAME LIKE '%' || '한라' || '%' ORDER BY bCnt DESC)A) C
WHERE RN BETWEEN 1 AND 3;

SELECT c.*, (SELECT COUNT(*) FROM BOOKMARK WHERE MID = 'kim' AND SNAME = C.sname) bookmarkOk
    				FROM (SELECT ROWNUM RN, A.* FROM (SELECT S.*, NVL(bCnt, 0) bCnt, L.LNAME FROM SPOT S, LOCATION L, (SELECT SNAME, COUNT(*) bCnt FROM BOOKMARK GROUP BY SNAME)B 
        		WHERE S.SNAME = B.SNAME(+) AND S.LOCATIONNO = L.LOCATIONNO AND S.LOCATIONNO IN(1,3,13) ORDER BY bCnt DESC)A) C
                WHERE RN BETWEEN 1 AND 1 AND C.SNAME LIKE '%' || '한라' || '%' ;
...
-- 5. 관광지 상세보기
SELECT ROWNUM RN, A.* FROM (SELECT S.*, NVL(bCnt, 0) bCnt, L.LNAME FROM SPOT S, LOCATION L, (SELECT SNAME, COUNT(*) bCnt FROM BOOKMARK GROUP BY SNAME)B 
        WHERE S.SNAME = B.SNAME(+) AND S.LOCATIONNO = L.LOCATIONNO AND S.SNAME = '한라산국립공원')A;
