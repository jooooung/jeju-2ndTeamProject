...
-- 지역
SELECT *
    FROM (SELECT ROWNUM RN, A.* FROM (SELECT R.*, NVL(bCnt, 0) bCnt, L.LNAME FROM restaurant R, LOCATION L, (SELECT RNAME, COUNT(*) bCnt FROM BOOKMARK GROUP BY RNAME)B
    WHERE R.RNAME = B.RNAME(+) AND R.LOCATIONNO = L.LOCATIONNO AND R.LOCATIONNO IN (1,3,13) AND R.RNAME LIKE '%' ||'델'|| '%' ORDER BY bCnt DESC)A)
    WHERE RN BETWEEN 1 AND 5;
...
SELECT COUNT(*) FROM restaurant WHERE LOCATIONNO IN (1,3,13) AND RNAME LIKE '%'||'제주'||'%';
