...
SELECT * 
    FROM (SELECT ROWNUM RN, A.* FROM (SELECT S.*, NVL(bCnt, 0) bCnt FROM SPOT S, LOCATION L, (SELECT SNAME, COUNT(*) bCnt FROM BOOKMARK GROUP BY SNAME)B 
        WHERE S.SNAME = B.SNAME(+) AND S.LOCATIONNO = L.LOCATIONNO AND S.LOCATIONNO IN(1,2,3) AND S.SNAME LIKE '%' || '협재' || '%' ORDER BY bCnt DESC)A)
WHERE RN BETWEEN 1 AND 3;
